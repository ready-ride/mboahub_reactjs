      ...
      <div className="container">
        <div className="latest-property-header">
          ...
          <div className="latest-property-header-right my-2 rounded">
            <span onClick={() => setItem('all')} className={allStyle}>All Categories</span>
            <span onClick={() => setItem('for_sale')} className={forSaleStyle}>For Sale</span>
            <span onClick={() => setItem('for_rent')} className={forRent}>For rent</span>
          </div>
        </div>
        <div className="latest-property-items row">
          {
                 houseloading
                   ...
                   : houses && houses.map((house, i) => i < 8 && <div key={i} className="col-sm"><PropertyItem house={house} key={house.id} /></div>)
                }
        </div>
