...
export const useMessages = () => {
 ...
 const handleSendMessage = (data) => {
  setError();
  let url = isLoggedIn ? MESSAGE_CREATE_URL : ANONYMOUS_MESSAGE_URL;

  (async() => {
    setMessageLoading(true);
    try{
      let res = await postRequestWithToken(data, token, url);
  ...
  return { error, response, messageloading, handleSendMessage };
};
