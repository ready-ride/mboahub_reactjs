...
export const useUserDetails = () => {
  ...
  if (!token) {
    ...
    useEffect(() => {
      ...
      (async() => {
        try{
          let res = await getRequestWithToken(token, USER_DETAILS_URL);
          if (res.success) {
            setData(res.data);
            setLoading(false);
          } else {
    ...
    const handleUpdate = (e, next) => {
      ...
      (async() => {
        try{
          let res = await putRequestWithToken(updateData, token, USER_DETAILS_URL);
          if (res.success) {
            setData(res.data);
            next(false);
          } else {
    ...
    console.log(updateData);
      (async() => {
        try{
          let res = await putRequestWithToken(updateData, token, PASSWORD_UPDATE_URL);
          if (res.success) {
            setData(res.data);
    ...
    return { handlePasswordChange, handleChange, handleSocialsUpdate, handlePersonalInfoUpdate , socialsLoading, personalInfoLoading, passwordLoading, errors, loading, data };
}
